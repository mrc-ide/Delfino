...
parser.add_argument('--logit_bias', type=float, default=0.0) # Set to 0.0 in run_experiment.py
...
# --- 3. HARD-CODED BIAS VECTOR SETUP ---
# Create a full-vocabulary tensor initialized with the default (usually 0.0)
logit_bias_vector = torch.full((VOCAB_SIZE,), args.logit_bias, device=DEVICE)

# Define target codes to receive the 'Massive' penalty
targets = ['E10', 'I50', 'I21', 'I63', 'N18']
massive_penalty = -100.0

# Identify indices and overwrite the vector
for i, label in enumerate(labels_list):
    if any(label.startswith(p) for p in targets) and i != T_DEATH_ID:
...
def simulate_patient(patient_id, apply_glp1):
    ...
    for year in range(args.time_horizon):
        ...
        x = torch.tensor(identity + history[-(48-len(identity)):], dtype=torch.long, device=DEVICE)[None, ...]
        ...
        with torch.no_grad():
            ...
            # Applying the pre-calculated vector
