...
# --- 1. ARGUMENT PARSING ---
parser = argparse.ArgumentParser() # <--- CRITICAL LINE
...
# Global arrays for O(1) weight/cost access
...
for _, row in disease_df.iterrows():
    tid = int(row['TokenID'])
    GLOBAL_WEIGHTS[tid], GLOBAL_COSTS[tid], IS_DISEASE[tid] = row['Weight'], row['Cost'], True
...
def simulate_patient(patient_id, apply_glp1):
    ...
    for t in context:
        if IS_DISEASE[t]:
            ...
            if not already_diagnosed[t]:
                active_weights.append(GLOBAL_WEIGHTS[t]); active_costs.append(GLOBAL_COSTS[t])
    ...
    for year in range(args.time_horizon):
        total_yld += sum(active_weights)
        qalys_add += max(0, 1.0 - sum(active_weights))
        u = 1.0
        for w in active_weights: u *= (1.0 - w)
        qalys_mult += u
        ...
        if IS_DISEASE[next_token] and not already_diagnosed[next_token]:
            active_weights.append(GLOBAL_WEIGHTS[next_token]); active_costs.append(GLOBAL_COSTS[next_token])
